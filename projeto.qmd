...
\noindent
Minha pesquisa visa aplicar o método de estimação de regressão quantílica no problema da reconciliação ótima de previsões de séries temporais hierárquicas ou agrupadas. Além disso, pretendo evidenciar as vantagens e problemas do método o comparando com as abordagens via MQO, MQP e MinT, adotando tanto a mediana quanto quantis extremos. O experimento será realizado nas séries temporais de saldo de crédito do Banco do Estado do Espírito Santo SA, organizadas hierarquicamente por superintendências regionais e agências, e agrupadas por modalidades de crédito selecionadas.
...
Utilizando a base de dados do Google Acadêmico, pesquisei pelas palavras-chave "*hierarchical forecast reconciliation*", encontrando 27.600 resultados. Utilizando um script para ordenar os resultados pelo número de citações^[https://github.com/WittmannF/sort-google-scholar], verifiquei que o trabalho mais citado é @hyndman2021.

```{r citacoes}
#| tbl-cap: Trabalhados mais citados com os termos "hierarquical forecast reconciliation"
...
Para a introdução, o planejado é detalhar os conceitos de séries temporais hierárquicas e agrupadas, coerência e reconciliação ótima notação matricial. Além disso, introduzir o conceito envolvendo a matriz $\mathbfit{G}$ e as abordagens de reconciliação clássicas (*bottom-up* e *top-down*) também na notação matricial. Paralelamente, já iniciarei a escolha das séries a serem trabalhadas e a formalização do uso e publicação da base de dados.
...
No capítulo 2, vou trazer os conceitos da regressão quantílica e aplicá-los ao problema da reconciliação ótima, que é o objetivo principal do trabalho. Trabalharei tanto com a mediana quanto com quantis extremos.

No capítulo 3, aplicarei os métodos aos dados de crédito do Banestes, irei comparar a performance com os demais métodos e vou analisar onde ele se mostrou mais ou menos vantajoso.
